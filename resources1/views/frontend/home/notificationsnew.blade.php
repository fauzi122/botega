                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="sidebar-search">
                            <form action="{{url('notifications')}}" method="get">
                                <input type="search" name="cari" value="{{$cari}}" placeholder="Pencarian...">
                                ...
                                <div class="accordion row" id="shippingInfo">
                                    @foreach($list as $l)
                                        ...
                                        <a href="{{url('ceknotif/'.Crypt::encrypt($l->id))}}">
                                            <div
                                                style="border: 1px solid #cccce0; border-radius: 8px; padding: 15px; margin: 5px; background-color: {{ $l->status == 0 ? 'ghostwhite' : 'white' }}">
                                                <div>
                                                    <h4>{{$l->actions}}</h4>
                                                    <p>{{$decodedPayload->description}}
                                                        <br>
                                                        <i class="fa fa-tags"></i> {{ Carbon::parse($l->created_at)->locale('id_ID')->isoFormat('D MMMM YYYY, HH:mm:ss') }}
                                                        |
                                                        @if($l->status == 0)
                                                            <span class="text-danger"><i class="fa fa-eye-slash"></i> Belum dibaca</span>
                                                        @else
                                                            <span class="text-success"><i class="fa fa-eye"></i> Sudah dibaca</span>
                                                        @endif
            ...
            <div class="pagination-wrapper">
                <ul>
                    ...
                    @if($list->currentPage() < $list->lastPage() - 2)
                        <li class="disabled"><span>...</span></li>
                        <li>
                            <a href="{{ $list->url($list->lastPage()) }}{{ !empty($cari) ? '&cari=' . $cari : '' }}">{{ $list->lastPage() }}</a>
                        </li>
                    @endif
