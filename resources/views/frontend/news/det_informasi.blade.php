...
{{--                    </div>--}}
                    <div class="col-lg-12 order-1">
                        ...
                        <div class="blog-single-post-details-wrapper">
                            ...
                            <p class="post-meta">By <a href="#" class="post-author">{{$l->first_name.' '. $l->last_name}}</a> <span class="separator">|</span> <a href="#">{{ \Carbon\Carbon::parse($l->published_at)->locale('id_ID')->isoFormat('D MMMM YYYY') }}</a><span class="separator">|</span> {{$l->hit}} Dilihat</p>

                            <div class="post-thumbnail">
                                <img src="{{url('article/image/'.$l->id.'.png')}}" class="img-fluid" alt="" style="border-radius: 20px; width: 100%; height: 500px; object-fit: cover"  onerror="this.src='{{ asset('assets_frontend/img/noimage.png') }}'">
                            </div>
                            ...
                            <div class="post-share-section">
                                ...
                                <ul class="post-social-icons">
                                    <li><a href="https://www.facebook.com/sharer/sharer.php?u={{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}" target="_blank"><i class="fa fa-facebook"></i></a></li>
                                    <li><a href="https://twitter.com/intent/tweet?url={{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}" target="_blank"><i class="fa fa-twitter"></i></a></li>
                                    <li><a href="https://plus.google.com/share?url={{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}" target="_blank"><i class="fa fa-google-plus"></i></a></li>
                                    <li><a href="https://pinterest.com/pin/create/button/?url={{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}" target="_blank"><i class="fa fa-pinterest"></i></a></li>
                                    <li><a href="https://api.whatsapp.com/send?text={{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
                                        ...
                                        @foreach($comment as $com)
                                            <div class="single-blog-comment">
                                                ...
                                                <div class="single-blog-comment__content">
                                                    <div class="comment-time"><i class="fa fa-calendar"></i> {{ \Carbon\Carbon::parse($l->created_at)->locale('id_ID')->isoFormat('D MMMM YYYY, H:mm') }} </div>
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <div class="contact-form-wrapper">
                                        <form action="{{url('post-comment')}}" method="post">
                                            @csrf
                                            <div class="row">
                                                <input type="hidden" name="id" value="{{\Illuminate\Support\Facades\Crypt::encrypt($l->id)}}">
                                                <div class="col-lg-12">
                                                    <textarea cols="30" rows="5" placeholder="Message *" name="comment" required></textarea>
                                                </div>
                                                <div class="col-lg-12">
                                                    <button type="submit" id="submit" class="theme-button"> ADD COMMENT</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
