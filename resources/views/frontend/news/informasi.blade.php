                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="single-blog-post">
                                            <div class="single-blog-post__image">
                                                <a>
                                                    <img src="{{ url('article/image/' . $l->id . '.png') }}"
                                                        class="img-fluid" alt=""
                                                        style="aspect-ratio: 1/1; width: 100%; object-fit: cover"
                                                        onerror="this.src='{{ asset('assets_frontend/img/noimage.png') }}'">
                                                </a>
                                            </div>
                                            <div class="single-blog-post__content p-3">
                                                <h3 class="post-title"><a
                                                        href="{{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}">{{ $l->judul }}</a>
                                                </h3>
                                                <p class="post-meta">By <a href="#"
                                                        class="post-author">{{ $l->first_name . ' ' . $l->last_name }}</a>
                                                    <span class="separator">|</span> <a
                                                        href="#">{{ \Carbon\Carbon::parse($l->published_at)->locale('id_ID')->isoFormat('D MMMM YYYY') }}</a>
                                                    <span class="separator">|</span> {{ $l->hit }}x
                                                </p>
                                                <p class="post-excerpt" style="text-align: justify">{!! \Illuminate\Support\Str::limit(strip_tags($l->article), 300) !!}
                                                </p>
                                                <a href="{{ url('detail-article/' . \Illuminate\Support\Facades\Crypt::encrypt($l->id)) }}"
                                                    class="blog-readmore-link">Read more <i
