...
(function ($) {
    ...
    function createMenu(){
        let baseurl = baseURL();
        let menu = [
            {"class":"menu-title", "data-key":"t-menu", "title":"Menu"},
            {"title":"Dashboard", "icon":"home", "url":baseurl + "/dashboard/"},
            {"title":"Master", "icon":"grid",
                "sub-menu" : [
                    { "title":"Bank", "url": baseurl + "/bank/" , 'icon' :'arrow-right-circle' },
                    { "title":"Cabang", "url": baseurl + "/cabang/", 'icon' :'git-branch'  },
                    { "title":"Level Member", "url": baseurl + "/level/", 'icon' :'arrow-right-circle'  },
                    { "title":"Approval", "url": baseurl + "/approval/", 'icon' :'user-check'  },
                    { "title":"Member", "url": baseurl + "/member/", 'icon' :'users'  },
                    // { "title":"Promo", "url": baseurl + "/promo/"  },
                ]
            },
            {"title":"Reward & Gift", "icon":"gift",
                "sub-menu" : [
                    { "title":"Gift Type", "url": baseurl + "/gift-type/" , 'icon' :'arrow-right-circle' },
                    { "title":"Gift", "url": baseurl + "/gift/", 'icon' :'arrow-right-circle'  },
                    { "title":"Reward", "url": baseurl + "/reward/", 'icon' :'arrow-right-circle'  },
                    { "title":"Redeem Point", "url": baseurl + "/redeem/", 'icon' :'arrow-right-circle'  },
                ]
            },
            {
                "title": "Konten Web", "icon": "layout",
                "sub-menu": [
                    { "title":"Kategori", "url": baseurl + "/kategori/", 'icon' :'arrow-right-circle' },
                    { "title":"Artikel", "url": baseurl + "/artikel/", 'icon' :'arrow-right-circle' },
                    { "title":"Event", "url": baseurl + "/event/", 'icon' :'arrow-right-circle' },
                    { "title":"Slider", "url": baseurl + "/slider/", 'icon' :'arrow-right-circle' },
                    { "title":"Video Youtube", "url": baseurl + "/video/", 'icon' :'youtube' },
                    { "title":"Katalog Produk", "url": baseurl + "/katalog-produk/", 'icon' :'trello' },
                ]
            },
            {
                "title": "Penjualan", "icon": "shopping-cart",
                "sub-menu": [
                    { "title":"Kategori Produk", "url": baseurl + "/kategori-produk/", 'icon' :'paperclip' },
                    { "title":"Produk", "url": baseurl + "/produk/" , 'icon' :'arrow-right-circle'},
                    { "title":"Penjualan", "url": baseurl + "/penjualan/" , 'icon' :'shopping-bag'},
                    { "title":"Fee Member", "url": baseurl + "/fee/" , 'icon' :'tag'},
                    { "title":"Payment Made", "url": baseurl + "/payment-made/" , 'icon' :'credit-card'},
                    { "title":"Outstanding Fee", "url": baseurl + "/fee/report-outstanding/" , 'icon' :'bar-chart'},
                ]
            },
            {
                "title": "Laporan", "icon": "file-text",
                "sub-menu": [
                    { "title":"Fee Per Member", "url": baseurl + "/report/fee-member/" , 'icon' :'users'},
                    { "title":"Fee Per Kategori", "url": baseurl + "/report/fee-kategori/" , 'icon' :'shopping-bag'},
                    { "title":"Fee Per Merk", "url": baseurl + "/report/fee-merk/" , 'icon' :'flag'},
                ]
            },
            {
                "title": "System", "icon": "settings",
                "sub-menu": [
                    { "title":"Hak Akses", "url": baseurl + "/hak-akses/", 'icon' :'arrow-right-circle' },
                    { "title":"Pengguna", "url": baseurl + "/pengguna/", 'icon' :'arrow-right-circle' },
                    { "title":"Log Aktifitas", "url": baseurl + "/log/", 'icon' :'arrow-right-circle' },
                    { "title":"Pengaturan", "url": baseurl + "/settings/", 'icon' :'arrow-right-circle' },
                ]
            }
        ];

        $("#side-menu").html(renderMenu(menu));
    }

    function renderMenu(menu, isubmenu=false){
        var tmp = "";
        menu.forEach(function(v,i,a){

            let key = (v.title ?? '').replaceAll(' ','').toLowerCase();
            let submenu = Array.isArray( v['sub-menu'] );

            tmp += `<li class="${v.class ?? ''}" data-key="${key}">`;
            if( v.class === 'menu-title' ){
                if(v.title !== undefined){
                    tmp += v.title;
                }
            }else{
                let classname = submenu ? 'has-arrow' : "";
                let iclass = isubmenu === true ? '' : 'icon-med';
                let icon = v.icon === undefined ? '' : `<i class="${iclass}" data-feather="${v.icon}"></i>`;
                tmp += `<a href="${v.url ?? "javascript:void(0)"}">${icon} <span class="${classname}" data-key="${key}">${v.title ?? ''}</span></a>`;
            }
            if( submenu  ){
                tmp += `<ul aria-expanded="false">`;
                tmp += renderMenu(v['sub-menu'], true);
                tmp += `</ul>`;
            }
            tmp += `</li>`;
        });
...
function baseURL(url = ''){
    return  $("meta[name=baseurl]").attr("content") + "/admin" + url;
}
...
function showModalLogout(){
    Swal.fire({
        ...
        if(e.value === true){
            let baseurl = baseURL();
            $.post(baseurl + "/auth", { "_method": "delete", "_token":csrf_token() }).done(function(e){
