...
function buildTable(){
    $('#jd-table').DataTable({
        ...
        buttons:[
            ...
            },
            {
                ...
                action: (e,dt,node, c)=>{
                    ...
                    showConfirm('jd-table', "Pengajuan Fee", "Pastikan data member yang akan di usulkan sudah benar untuk dilakukan proses pengajuan?",(e, id)=>{
                        if(e.value){
            ...
            'csv', 'copy',
            {
                ...
                action: (e,dt,node, c)=>{
                    window.open( baseurl() + '/admin/fee/download/fee.xls' ,'frame-download');
                },
        ...
        order: [[1, 'asc']],
        columnDefs:[
            {targets:[5,6,7,8,9,10], className:'dt-right'}
        ],
        columns:[
            ...
            { data:'first_name', render:(data, type, row, meta) => {
                    return `${data} ${row['last_name']} (${row['id_no']})<br/><small class="badge badge-soft-info">${row['kategori'] ?? '-'}</small>`;
                }},
            ...
            { data: 'pph_amount',  render:(data, type, row, meta) => {
                    return Number(row['is_perusahaan']) === 1 ? '-' : formatUang(data);
                }},
            { data: 'pph_amount', render:(data, type, row, meta) => {
                    return Number(row['is_perusahaan']) === 1 ?  formatUang(data) : '-';
                }},
    ...
    $('#jd-table-pengajuan').DataTable({
        ...
        buttons:[
            {
                ...
                action: (e,dt,node, c)=>{
                    ...
                    showConfirm('jd-table-pengajuan', "Proses Fee", "Pastikan data member yang akan di usulkan untuk di proses sudah benar?",(e, id)=>{
                        if(e.value){
                            $.post(url + '/status/acc', {
                                '_token': csrf_token(),
                                'id': id
            ...
            'csv', 'copy', {
                ...
                action: (e,dt,node, c)=>{
                    ...
                    $('#jd-table-pengajuan_processing').css('display','block');
                    window.open( baseurl() + '/admin/fee/download/fee-pengajuan.xls' ,'frame-downloads');

                    setTimeout(function(){
                        $('#jd-table-pengajuan_processing').css('display','none');
                    }, 3000);
                },
                className: 'btn-success'
            },
            ...
            }, 'print',
            {
                text:'<i class="mdi mdi-trash-can-outline"></i> Hapus',
                action: (e,dt,node, c)=>{
                    let url =  $('#jd-table-pengajuan').data('urlaction') + '/remove/pengajuan';
        ...
        columns:[
            ...
            {
                data:'fee_number_id', render:(data, type, row, meta) => {
...
*/
    ...
    $('#jd-table-setujui').DataTable({
        ...
        buttons:[
            {
                ...
                action: (e,dt,node, c)=>{
                    ...
                    showConfirm('jd-table-setujui', "Selesai Pembayaran", "Pastikan data member yang akan di usulkan untuk di selesai sudah benar?",(e, id)=>{
                        if(e.value){
            ...
            },
            {
                ...
                action: (e,dt,node, c)=>{
                    ...
                    let url = baseurl() + `/admin/fee/download-csv/fee-disetujui.xls?id=${JSON.stringify(ids)}`;
                    console.log(url);
            ...
            'copy', {
                text:'<i class="mdi mdi-file-excel"></i>',
                action: (e,dt,node, c)=>{
                    window.open( baseurl() + '/admin/fee/download/fee-acc.xls' ,'frame-download');
                },
                className: 'btn-success'
            },
            {
            ...
            }, 'print',
            {
                text:'<i class="mdi mdi-trash-can-outline"></i> Hapus',
                action: (e,dt,node, c)=>{
                    let url =  $('#jd-table-setujui').data('urlaction') + '/remove/acc';
        ...
        columns:[
            ...
            { data:'nomor', render:(data, type, row, meta) => {
                return `${data}<br/><small class="badge badge-soft-success">${row['kode_merger'] ?? ''}</small>`;
            }},
            { data:'periode'},
            { data:'first_name', render:(data, type, row, meta) => {
                    return `${data} ${row['last_name']} (${row['id_no']})<br/><small class="badge badge-soft-info">${row['kategori'] ?? '-'}</small>`;
            ...
            {
                data:'nama_bank', render:(data, type, row, meta)=>{
                    return `${row['nama_bank'] ?? '[Bank not set]'} - ${row['no_rekening'] ?? '[account bank not set]'}<br><span class="badge badge-soft-primary">${row['an_rekening'] ?? 'Not set'}</span>`;
                }
            },
    ...
    $('#jd-table-selesai').DataTable({
        ...
        buttons:[
            'csv', 'copy', {
                text:'<i class="mdi mdi-file-excel"></i>',
                action: (e,dt,node, c)=>{
                    window.open( baseurl() + '/admin/fee/download/fee-finish.xls' ,'frame-download');
                },
                className: 'btn-success'
            },
            {
            ...
            , 'print',
            {
                text:'<i class="mdi mdi-trash-can-outline"></i> Hapus',
                action: (e,dt,node, c)=>{
                    let url =  $('#jd-table-selesai').data('urlaction') + '/remove/selesai';
        ...
        columns:[
            ...
            { data:'first_name', render:(data, type, row, meta) => {
                    return `${data} ${row['last_name']} (${row['id_no']})<br/><small class="badge badge-soft-info">${row['kategori'] ?? '-'}</small>
                            <small class="badge badge-soft-success">${row['kode_merger'] ?? ''}</small>`;
                }},
            { data: 'npwp', render:(data, type, row, meta) => {
                    return data?.length > 7 ? 'Ya' : 'Tidak';
                }},
            ...
            {
                data:'no_faktur', render:(data, type, row, meta)=>{
...
function editdata(id){
    ...
    wire.edit(id).then(()=>{
        ...
        $('select#member_user_id').html(`<option value="${id}">${name}</option>`);
