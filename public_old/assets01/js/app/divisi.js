...
$(function(){
    ...
    var jdtable = $('table#jd-table').DataTable({
        ...
        columns:[
            ...
            { data:'division', 'width':'150px', render:function(data,type, row, meta){
            ...
            {
                data: 'pathname', render: function (data, type, row, meta) {
    ...
    edit = function(id){
        ...
        var url = `${baseurl}/divisi/` + id;
       $.httpGet(url, {
            onSuccess: (d)=>{
                console.log('edit : ',url,d);
                var data = $.htmlentitiesdecode(d.parent_pathname) ?? '';
                var path = $.implode(' / ', data);
                $('select[name=parent_id]').append(`<option value="${d.parent_id}">${path}</option>`);
