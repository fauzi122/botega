...
$(function(){
    ...
    var jdtable = $('table#jd-table').DataTable({
        ...
        columns:[
            ...
            { data:'category', 'width':'150px', render:function(data,type, row, meta){
            ...
            { data: 'typ_media'},
            { data: 'full_path', render: function (data, type, row, meta) {
    ...
    edit = function(id){
        ...
        var url = `${baseurl}/kategori/` + id;
       $.httpGet(url, {
            onSuccess: (d)=>{
                console.log('edit : ',url,d);
                var data = $.htmlentitiesdecode(d.full_path) ?? '';
    ...
    confirmHapus = function(){
        swal({
            ...
            if(r){
                var baseurl = $('meta[name="baseurl"]').attr('content');
                var url = `${baseurl}/kategori/delete`;
                $.httpPost( url,
