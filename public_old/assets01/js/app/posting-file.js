    ...
    var jdtablefile = $('table#jd-tablefile').DataTable({
        ...
        columns:[
            ...
            { data:'filename', render:function(data,type,row){
                    ...
                    var url = $.baseURL() + `/repo/files/${row['id']}/download/${fname}`
                    var btncopylink = `<br/><a href="void:javascript()" class="text-info" onclick="copylink('${url}')"><i class="fa fa-copy"></i> Salin tautan unduhan</a>`;
                    ...
                    return `<a title="unduh file ${data}" data-toggle="tooltip" class="${url}" href="${url}"><i class="fa fa-download"></i> ${data}</a> <br/>${btncopylink} ${btnunzip}`;
                }
            },
            { data:'last_download', render:function(data,type,row){
                    if(data === null)return '';
                    var d = new Date(data);
                    return moment(d).format('ddd, DD/MMM/YYYY,<br/>HH:mm');
                }},
            { data:'hits'},
