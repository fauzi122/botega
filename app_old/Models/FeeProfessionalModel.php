...
class FeeProfessionalModel extends Model
{
    ...
    public static function view(){
        return FeeProfessionalModel::query()->from(function (Builder $b){
            return $b->from('fee_professional as a')
                     ...
                     ->select(['a.*', 'fn.nomor', 'u.id_no', 'u.first_name', 'u.last_name', 'p.history_number as invoice_number',
                            \DB::raw('concat(utrx.first_name," " ,utrx.last_name) as customer '),
                            'psj.history_number as sj_number', "dt.item_disc_percent", "km.name as kategori",
                         \DB::raw("SUBSTRING_INDEX(pd.name, ' ', 1) as merk"),
                            \DB::raw('concat(year(invoice_date), "-",month(invoice_date)) as periode'),
                             'dt.salesname', 'trx.nomor_so',
                             \DB::raw('COALESCE(fn.payment_made, 0) as payment_made'),
                             'dt.sale_price', 'dt.ppn', 'dt.qty', 'u.is_perusahaan', 'pd.name as product',
                            'dt.product_id as product_id', 'pc.category', 'pd.category_id'
                         ]);
        }, 'fee_professional');
    }
    ...
    public static function resume(){
        return self::view()->select([
            'fee_number_id', 'nomor', 'member_user_id', 'id_no', 'first_name', 'last_name','product', 'periode', 'kode_merger',
            'npwp','kategori',
            \DB::raw('max(payment_made) as payment_made'),
            \DB::raw('sum(dpp_amount) as dpp_amount'),
            \DB::raw('sum(fee_amount) as fee_amount'), 'fee_percent', 'pph_percent' ,
            \DB::raw('sum(pph_amount) as pph_amount'),
            \DB::raw('sum(total_pembayaran) - max(payment_made)  as total_pembayaran'), 'is_perusahaan'
    ...
    public static function resumeRekening(){
        return self::view()->select([
            ...
            \DB::raw('sum(pph_amount) as pph_amount'),
            \DB::raw('sum(total_pembayaran) - max(payment_made)  as total_pembayaran'), 'is_perusahaan',
            'nama_bank', 'no_rekening','an_rekening', 'no_faktur', 'bank_kota'
        ], 'fee_professional')->groupBy(['fee_number_id']);
    }
