...
class PenjualanProdukController extends Controller
{
    ...
    public function select2nomor_so(){
        $q = \request('q');
        $r = QueryBuilderExt::whereFilter(TransactionModel::query()
            ->whereNotIn('status',['QUEUE', 'DRAFT','UNAPPROVED'])
            ->orderBy('trx_at', 'desc') , ['nomor_so'],
            $q
        )->paginate(10);
        $ret[] = ['id'=>'', 'text'=>'--'];
        foreach($r as $k){
    ...
    public function select2invoice(){
        $q = \request('q');
        $r = QueryBuilderExt::whereFilter(ProsesHistoryModel::query()
                ->whereRaw('LEFT(history_number,2)=?',['IN'])
                ->orderBy('history_date', 'desc') , ['history_number', 'history_date'],
            $q
        )->paginate(10);
        $ret[] = ['id'=>'', 'text'=>'--'];
        foreach($r as $k){
