...
function createURLGridView(start = 0) {
    ...
    let url =
        protocol + window.location.host + "/admin/katalog-produk/data-source/?";
    ...
    $.get(createURLGridView(page)).done(function (E) {
        ...
        for (var idx in data) {
            ...
            let filekatalog =
                n["fileunduh"] === ""
                    ? ""
                    : `<a href="${n["fileunduh"]}" target="_blank"><i class="mdi mdi-download"></i> Unduh file</a>`;
            body += `<div class="col-md-3">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                         <small class="text-muted"> ${filekatalog} ${
                n["created_at"] ?? n["updated_at"] ?? ""
            }</small>
                                    </div>
                                    <div class="col-md-6 ">
                                        <button data-toggle="tooltip" onclick="editdata('${
                                            n["id"]
                                        }')" title="Edit" class="btn btn-sm btn-outline-info"><i class="mdi mdi-pencil"></i></button>
                                        <button data-toggle="tooltip" title="Hapus" onclick="hapusdata('${
                                            n["id"]
                                        }')" class="btn btn-sm btn-outline-danger"><i class="mdi mdi-trash-can"></i></button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function editdata(id) {
    ...
    wire.edit(id).then(() => {
        ...
        if (pathimage === null || pathimage === undefined || pathimage === "") {
        } else {
