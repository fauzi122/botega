...
$(function(){
    ...
    var jdtable = $('table#jd-table').DataTable({
        ...
        columns:[
            ...
            { data:'nama_group', 'width':'150px', render:function(data,type, row, meta){
    ...
    edit = function(id){
        ...
        var url = `${baseurl}/grouparchive/` + id;
       $.httpGet(url, {
            onSuccess: (d)=>{
    ...
    formValidator = function(){
        return {
            'rules' : {
                'nama_group': { 'required' :true, 'minlen':5 },
            }
        };
    };
    ...
    confirmHapus = function(){
        swal({
            ...
            if(r){
                var baseurl = $('meta[name="baseurl"]').attr('content');
                var url = `${baseurl}/grouparchive/delete`;
                $.httpPost( url,
