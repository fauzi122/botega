    ...
    $('#jd-table').DataTable({
        ...
        columns:[
            ...
            {data:'first_name', render:(data, type, row, meta)=>{
                return `${data} ${row['last_name']}<br/><small class="badge badge-soft-info" >${row.kategori ?? ''}</small> `;
            }},
            {data:'level_name', render:(data,type, row, meta)=>{
                ...
                return `${label} ${data ?? ''}<br/><small class="badge badge-soft-success">IDR ${totalspent}</small>`
            }},
            {data:'hp', render:(data, type, row, meta) => {
                const sudahverifi = row['date_verify_email'] === null ? "" : `<i class="mdi mdi-check-all" style="color: green"></i>`;
                return `${data ??''}<br/>${row['wa'] ?? ''}<br/>${row['email'] ?? '-'} ${sudahverifi}`;
            }},
            { data: 'last_name', sortable:false, width:'100px', render:(data, type, row, meta) => {
...
function editdata(id){
    ...
    wire.edit(id).then(()=>{
        ...
        $('input[name=memberCheckbox]').prop('checked',reward === 1 || reward === 3);
        $('input[name=umumCheckbox]').prop('checked',reward === 2 || reward === 3);
    });
}
function save(){
